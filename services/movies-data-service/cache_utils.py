...
SET_ROOMS = "rooms"
SET_MOVIES = "movies"
SET_SCREENINGS_BY_MOVIE = "movie_screenings"
...
def invalidate_movies():
    r.delete(SET_MOVIES)
    for key in r.scan_iter(f"{PREFIX_MOVIE}:*"):
        r.delete(key)

def invalidate_rooms():
    r.delete(SET_ROOMS)
    for key in r.scan_iter(f"{PREFIX_ROOM}:*"):
        r.delete(key)

def invalidate_screenings():
    for key in r.scan_iter(f"{SET_SCREENINGS_BY_MOVIE}:*"):
        r.delete(key)
    for key in r.scan_iter(f"{PREFIX_SCREENING}:*"):
        r.delete(key)
